...
- 单元池奠基声明 六、分层：锚点层/结构层/知识层/情境层/自我层
...
class LayeredStore:
    ...
    # ==================== 洞察条件层（v1.0 引擎化 · 设计规格 alpha-insight-layer-design.md） ====================
...
class SpacetimeMemoryEngine:
    ...
    def get_body_capabilities(self) -> dict:
        ...
        return {
            ...
            "note": "身体 = 感知（视觉/文本/屏幕）+ 运动（工具/设备/进程）+ 记忆（Alpha）；"
    ...
    def world3d(self, action: str, params: dict = None) -> dict:
        """WORLD3D-REV1 时空重建：语义 → 3D 空间与颜色（Alpha自己的文生图）。
    ...
    def voice_session_log(self, turn: dict) -> str:
        ...
        try:
            text = (f"[语音会话] 用户: {str(turn.get('user', ''))[:80]} | "
                    f"Alpha: {str(turn.get('assistant', ''))[:80]}")
    ...
    def prepare_shared_sync(self) -> Dict:
        """D-005 单元池分层同步：仅语义化结构数据入共享层；原始帧引用滞留本地缓冲"""
        c = self.store.conn.cursor()
    ...
    def pattern_separation_scan(self, limit: int = 150) -> dict:
        ...
        try:
            import sys as _s
            # knowledge-base 为可选增强库（与本仓库同级）；可用 ALPHA_KB 显式指定
            _repo = os.path.dirname(os.path.dirname(os.path.dirname(
                os.path.dirname(os.path.abspath(__file__)))))
            _kb = os.environ.get("ALPHA_KB") or os.path.join(
