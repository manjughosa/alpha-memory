# -*- coding: utf-8 -*-
"""
aeis.api · Alpha高层接口 — 面向其他智能体的接入点
=================================================
...
class Agent:
    ...
    def session_recall(self, session_id: str = None, query: str = None,
                       limit: int = 10) -> List:
        """会话要点恢复：检索Alpha中的会话记忆（按 session 或语义）。
    ...
    # ==================== 洞察条件层（设计规格 alpha-insight-layer-design.md） ====================
    ...
    # ---- 智慧之书桥接（alpha-wisdom · 知识之书入口） ----
    # Alpha调用智慧之书（条件论知识图谱）作为主打工具：分析/验证/组合/出招。
    ...
    def wisdom_chat(self, message: str, session_id: str = "default") -> Dict:
        ...
        大脑（Alpha）可直接发起对话，不需要独立 /chat 服务。
        v1.15 H1：注入海马体前馈——真问题先过新奇检测，高新奇当场强化编码。
        v1.15 S5：对话记忆接入Alpha长期层——session_note 写入（跨重启/跨 session
        持久），「记得/刚才」先查Alpha记忆再查进程 dict。"""
        dex = self._get_wisdom()
        try:
            import os as _os
            import sys as _s
            ...
            # 注入Alpha记忆召回器：chat() 的「记得/刚才」优先查Alpha长期层
            def _alpha_memory_recall(sess, query=None, limit=6):
                try:
            ...
            result = _ce.chat(dex, message, session_id=session_id,
                              ...
                              memory_recall_fn=_alpha_memory_recall)
            # S5：对话要点写入Alpha长期层（持久化，跨重启）
            ...
            try:
                ...
                self.engine.add_context(
                    f"[会话 {session_id}] 用户：{message[:80]}｜Alpha：{reply_snip}",
    ...
    def roleplay_chat(self, message: str, session_id: str = "default",
        ...
        """统一角色扮演对话入口（MCP / 网页共用，信息处理全部由Alpha完成）：
        白箱优先（诚实边界/自省/闲聊/知识）→ 角色扮演意图/白箱无把握 → LLM
        （注入角色条件空间/自我锚点/价值观 + 诚实边界机制）。

        依赖 aeis.roleplay_chat.AlphaChat（零外部依赖管线）。
        """
        try:
            from .roleplay_chat import AlphaChat
        except Exception:
            return {"reply": "角色扮演管线未就绪", "route": "error", "honest": True}
        try:
            lc = AlphaChat(data_dir=data_dir or "roleplay_data",
