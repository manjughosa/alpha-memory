# -*- coding: utf-8 -*-
"""Alpha · 因果候选生成器 causal_discover（v1.16 · R 维进化）
...
class CausalDiscoverer:
    ...
    def _declare_candidate(self, anomaly):
        ...
        return {
            ...
            "condition_space": {
                "observation_position": "Alpha行为观测层（被拒路径/预测反馈）",
...
if __name__ == "__main__":
    ...
    agent = Agent(identity="Alpha",
                  db_path=os.environ.get("ALPHA_DB") or os.path.join(
                      os.path.expanduser("~"), ".alpha-memory",
                      "alpha-memory.db"))
    # 注入一条被拒路径作为信号
    try:
