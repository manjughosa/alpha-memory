...
多帧双目 → 3D 轨迹 → Alpha记忆（[3D时空事件] spatial3d 标签）→ 3D 时空问答：
  「球往哪飞了？」→ x+；「飞多快？」→ 0.2单位/帧；「飞了多远？」→ 位移；「轨迹直吗？」→ 一致性
零 LLM 确定性——看见（3D）→ 记住（Alpha）→ 时空问答（3D）。
"""
import sys
...
def see_3d_and_remember(agent, stereo_seq, label):
    """看见→记住（3D）：多帧双目 → 3D 轨迹 → Alpha记忆（spatial3d 标签）
    返回 (motion, node)：3D 运动原语 + Alpha记忆节点"""
...
def answer_3d_from_content(qtype, content):
    """从 3D 记忆内容直接回答（真实库时间线模式：内容来自 alpha_timeline
...
def timeline_3d_answer(question, timeline_events, limit=6):
    ...
    timeline_events: alpha_timeline 输出（[{id, content, tags, ...}]）"""
    qtype = _classify_3d(question)
    if qtype is None:
        return {"ok": False, "reply": "（非 3D 时空问题——不属感知通道）", "type": None}
...
def verify_3d_remembered(agent, node, motion):
    """3D 记忆自校验：Alpha记忆节点与 3D 运动原语一致性（无幻觉）"""
...
if __name__ == "__main__":
    print("=== 3D 时空问答：看见(3D)→记住(Alpha)→问答（零 LLM）===\n")
