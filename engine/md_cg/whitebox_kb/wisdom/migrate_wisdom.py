...
# -*- coding: utf-8 -*-
"""
migrate_wisdom · 智慧之书 → Alpha主库迁移器（v1.16 · 统一信息差减少任务）

设计者方向（2026-08-18）：智慧之书库迁移进Alpha主库，Alpha成为完整工程实现
...
# 默认路径（可参数覆盖）；目标库走 ALPHA_DB 环境变量，否则本系统状态根下
DEFAULT_META_SRC = os.path.join(os.path.dirname(os.path.abspath(__file__)), "wisdom-book-cloud.db")
DEFAULT_DST = os.environ.get("ALPHA_DB") or os.path.join(
    os.path.expanduser("~"), ".alpha-memory", "alpha-memory.db")
...
class WisdomMigrator:
    ...
    def run(self, meta: bool = True, subject: bool = True,
        ...
        self.report["source"] = "migrate_wisdom v1.16（智慧之书→Alpha主库）"
        if persist:
