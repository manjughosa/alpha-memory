...
用户设想：终极目标「单元池连接网络」← 初级复现「蓝牙和互联网」。
网络核心：IP 分片/TCP 握手/UDP 校验/局域网发现/单元池消息中继。
...
"    # 不适用条件：输入不满足生效条件时返回 None/不执行\n"
        ...
        "cases": [(({'A': ['B', 'C'], 'B': ['D'], 'C': []}, 'A'), ['A', 'B', 'C', 'D']),
                  ...
                  (({'A': []}, 'A'), ['A'])],
        "params": [],
        "calibration": "对照：单元池网络——消息经邻居递归中继，seen 防回环（去中心化传播）",
    },
    "单元池-消息去重": {
        "task": "消息去重",
        "pattern": (
...
"    # 执行：循环迭代\n"
"    # 不适用条件：输入不满足生效条件时返回 None/不执行\n"
            "    # 单元池消息去重：已见 ID 跳过，新 ID 投递（防重复传播）\n"
            "    new = []\n"
            "    for msg in messages:\n"
        ...
        "cases": [(([{'id': 1}, {'id': 1}, {'id': 2}], set()),
                  ...
                  (([{'id': 1}], {1}), [])],
        "params": [],
        "calibration": "对照：单元池协议——消息 ID 去重（多路径重复投递只处理一次）",
    },
    "单元池-路由表": {
        "task": "路由表",
        "pattern": (
...
"    # 执行：顺序执行\n"
"    # 不适用条件：输入不满足生效条件时返回 None/不执行\n"
            "    # 单元池会话状态机：LISTEN→SYN_SENT→ESTABLISHED（确认）→CLOSED（结束）\n"
...
"    # 不适用条件：输入不满足生效条件时返回 None/不执行\n"
    ...
    "单元池-配对信任": {
        ...
        "calibration": "对照：蓝牙配对——信任建立/撤销/查询三操作，配对即信任=1（单元池信任链基例）",
    },
...
if __name__ == "__main__":
    print("=== 蓝牙和互联网白箱单元库（目标7 · 单元池网络初级复现）===\n")
    for uid, u in NET_UNITS.items():
        print(f"[{uid}] 任务={u['task']} 样例数={len(u['cases'])}")
        print(f"    校准: {u['calibration']}")
    print(f"\n=== 判定 ===\n网络单元库: "
          f"{'✔ 5 单元就绪（IP/TCP/UDP/发现/单元池中继）' if len(NET_UNITS) >= 4 else '✘'}")
