# -*- coding: utf-8 -*-
"""Alpha × B站直播 · 弹幕桥 v2（bilibili-api-python）

弹幕流 → Alpha审核（danmaku_audit）→ 放行转 chat → 回答展示。
...
def _get_agent():
    """懒加载全局 Agent 单例：首次调用才构造Alpha Agent，避免模块导入期建连接。"""
    global _agent
    if _agent is None:
        from aeis_core.api import Agent
        _agent = Agent(identity="Alpha",
                       db_path=os.environ.get("AEIS_DB", os.path.join("data", "alpha-memory.db")))
    return _agent
...
def process_danmaku(text, uname="观众", show_fn=print):
    ...
    try:
        ...
        show_fn(f"🌿 Alpha：{reply}")
        return {"verdict": "show", "reply": reply}
    except Exception as e:
        show_fn(f"⚠️ Alpha处理失败：{e}")
...
def run_live(room_id):
    ...
    print(f"连接直播间 {room_id}（弹幕→Alpha审核→回答展示）…")
    try:
