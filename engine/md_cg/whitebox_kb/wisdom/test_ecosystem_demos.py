...
# ============ 目标7 单元池连接网络（net：握手→分帧→会话→可靠传输） ============
try:
    handshake = get('写一个 TCP 握手单元（三次握手状态）', 'tcp_handshake')
    frame_decode = get('写一个单元池消息分帧单元（字节流分隔）', 'frame_decode')
    session = get('写一个单元池会话状态单元（连接建立关闭）', 'session_step')
    ...
    check('目标7 单元池连接网络: 握手→分帧→会话→窗口', ok7,
          f'hs={hs} state={state} win={win["window"]}')
except Exception as e:
    check('目标7 单元池连接网络: 握手→分帧→会话→窗口', False, str(e)[:50])
