...
# ⑩ 目标7 深化：单元池协议（去重/路由表/超时重传/停等 经正式管线）
n2_qs = {
    "去重": "写一个单元池消息去重单元（ID防重复）",
...
for label, q in n2_qs.items():
    ...
    if r.get("ok") and r.get("code") and "def " in r.get("code", ""):
        n2_ok += 1
    check(f'⑩ {label} 单元池协议单元经正式管线',
          r.get("ok") and "def " in r.get("code", ""),
          f'{r.get("unit")} | {(r.get("checks") or ["固化直出"])[0][:18]}')
check('⑩b 单元池四单元全部生成', n2_ok == 4, f'{n2_ok}/4')
...
# ⑭ 目标7 深化：单元池会话层（消息分帧/会话状态 经正式管线）
n3_qs = {
    "消息分帧": "写一个单元池消息分帧单元（字节流分隔）",
    "会话状态": "写一个单元池会话状态单元（连接建立关闭）",
}
n3_ok = 0
for label, q in n3_qs.items():
    ...
    if r.get("ok") and r.get("code") and "def " in r.get("code", ""):
        n3_ok += 1
    check(f'⑭ {label} 单元池会话层单元经正式管线',
          r.get("ok") and "def " in r.get("code", ""),
          f'{r.get("unit")} | {(r.get("checks") or ["固化直出"])[0][:18]}')
check('⑭b 单元池会话层二单元全部生成', n3_ok == 2, f'{n3_ok}/2')

# ⑭c 分帧+会话端到端（粘包拆帧→喂会话状态机）
r_frame = domain_route("写一个单元池消息分帧单元（字节流分隔）")
r_sess = domain_route("写一个单元池会话状态单元（连接建立关闭）")
try:
    ns1, ns2 = {}, {}
...
try:
    ...
    w = ns_wt["with_test"]()
    a = ns_at["attr_test"]()
    check('㊋c 装饰→上下文→属性端到端（timed5 进入退出 Alpha/None）',
          d == ('timed', 5) and w == (True, False) and a == ('Alpha', None),
          f'dec={d} with={w} attr={a}')
except Exception as ex:
    check('㊋c 装饰→上下文→属性端到端（timed5 进入退出 Alpha/None）', False, str(ex)[:60])
