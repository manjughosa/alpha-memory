...
export function buildInitializationRequest(input: Partial<InitializationRequest> = {}): InitializationRequest {
...
export function inspectInitialization(options: InitOptions, request = buildInitializationRequest()): CheckResult[] {
  const settingExists = existsSync(options.settingPath)
  let setting: any = null
  try { setting = loadAlphaDogSetting(options.settingPath) } catch { setting = null }
  ...
  const registryExists = registryCandidates.some((candidate: string) => {
    if (candidate.startsWith('@')) return Boolean(options.registry?.entries?.[candidate]?.path || options.registry?.[candidate]?.path)
...
export function createDeclinedState(): { status: 'declined'; mention: 'off' } {
  return { status: 'declined', mention: 'off' }
}

export async function initializeAlphaDog(options: InitOptions, requestInput: Partial<InitializationRequest> = {}) {
  const request = buildInitializationRequest(requestInput)
  const checks = inspectInitialization(options, request)
  const failed = checks.filter((item) => !item.ok)
  if (failed.length > 0) return { status: 'unconfigured' as const, mention: 'on' as const, request, checks, failed: failed.map((item) => item.name) }

  const verification = await runMinimalVerification(options, request)
  const verificationFailed = verification.filter((item) => !item.ok)
  if (verificationFailed.length > 0) return { status: 'failed' as const, mention: 'on' as const, request, checks, verification, failed: verificationFailed.map((item) => item.name) }

  const generated = writeInitializationArtifacts(options, request, checks, verification)
  return { status: 'configured' as const, mention: 'on' as const, request, checks, verification, generated }
}

export function writeDeclinedState(options: InitOptions) {
...
export async function runMinimalVerification(options: InitOptions, request: InitializationRequest): Promise<CheckResult[]> {
...
function writeInitializationArtifacts(options: InitOptions, request: InitializationRequest, checks: CheckResult[], verification: CheckResult[]) {
  const stateDir = resolve(options.root, 'state')
  mkdirSync(stateDir, { recursive: true })
...
function createModelModule(request: InitializationRequest) {
...
async function syntaxCheck(root: string): Promise<CheckResult> {
  const modelPath = join(root, 'model.js')
  if (!existsSync(modelPath)) return { name: 'node_syntax', ok: true, detail: 'model.js 将在初始化成功后生成' }
...
async function requestCheck(name: string, requestFn: InitOptions['transport'] extends infer T ? ((payload: unknown, config: Record<string, unknown>) => Promise<unknown>) | undefined : never, payload: unknown, config: Record<string, unknown>): Promise<CheckResult> {
  if (typeof requestFn !== 'function') return { name, ok: true, detail: 'transport 未注入，保留为宿主验证项' }
  try { await requestFn(payload, config); return { name, ok: true, detail: '请求成功' } } catch (error) { return { name, ok: false, detail: error instanceof Error ? error.message : String(error) } }
}

function canReadWrite(directory: string) {
  try {
...
function atomicJsonWrite(path: string, value: unknown) { atomicTextWrite(path, JSON.stringify(value, null, 2) + '\n') }
function atomicTextWrite(path: string, value: string) { const temp = `${path}.${randomUUID()}.tmp`; mkdirSync(dirname(path), { recursive: true }); writeFileSync(temp, value, 'utf8'); renameSync(temp, path) }
