...
export class AlphaDogWakeQueue {
  ...
  requeue(id: string) { const item = this.queue.find((entry) => entry.id === id); if (!item || item.status !== 'failed') return null; item.status = 'queued'; return item }
  requeueFailed() { let n = 0; for (const item of this.queue) if (item.status === 'failed') { item.status = 'queued'; n += 1 } return n }
