export function createBatches(slot: any, round: number, entries: any[], batchSize: number, tokenBudget = 0) {
  ...
  for (const entry of sorted) {
    ...
    if (fullByCount || fullByTokens) { chunks.push(current); current = []; tokens = 0 }
    current.push(entry)
    tokens += estimate
  }
...
function estimateTokens(entry: any) {
  if (Number.isFinite(Number(entry?.tokens))) return Math.max(1, Number(entry.tokens))
  if (Number.isFinite(Number(entry?.size))) return Math.max(1, Math.ceil(Number(entry.size) / 4))
