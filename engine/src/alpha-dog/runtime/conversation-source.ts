...
export function resolveConversationCandidates(declared: string, probeRoots: string[] = []): string[] {
  ...
  for (const root of roots) {
    for (const file of listJsonl(root)) candidates.push(file)
  }
  // 去重（保留声明优先）
...
export function readConversation(declared: string, probeRoots: string[] = [], maxFileBytes = 16 * 1024 * 1024): ConversationSourceResult {
  ...
  try {
    ...
    for (const line of raw.split(/\r?\n/)) {
      ...
      if (effectiveType === 'user') {
        ...
        if (text) {
          const turn = { kind: 'user' as const, text, at: ts || String(nested?.timestamp || ''), roundHint: userTurns.length + 1 }
          turns.push(turn)
          userTurns.push(turn)
        }
      } else if (effectiveType === 'assistant') {
        text = extractText(nested || obj)
        if (text) turns.push({ kind: 'assistant' as const, text, at: ts || String(nested?.timestamp || '') })
      } else if (effectiveType === 'toolResult' || effectiveType === 'tool_result' || effectiveType === 'tool') {
        text = extractText(nested || obj)
        if (text) turns.push({ kind: 'tool' as const, text, at: ts || String(nested?.timestamp || '') })
      }
    }
...
function extractText(obj: any): string {
  ...
  if (typeof obj.message?.content === 'string') return obj.message.content.trim()
  return ''
}
