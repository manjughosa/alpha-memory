...
export class AlphaBridge {
  ...
  private spawnAndHandshake(): void {
    ...
    proc.stderr?.on('data', (chunk: Buffer) => {
      ...
      if (text) console.error(`[alpha-memory-bridge] ${text}`)
    })
    ...
    proc.on('exit', (code, signal) => {
      ...
      if (!this.disposed) {
        ...
        if (uptimeS >= 0 && uptimeS < 5) {
          console.error(
            `[alpha-memory-bridge] 进程启动后 5 秒内即退出——请检查 ${python} 可执行文件与 md_cg 依赖。` +
            `自检 \`${selfCheckCommand(python)}\` 须在发行包根目录运行` +
            '（包已自动锚定 cwd 与 PYTHONPATH，issue #12）。')
        } else if (this.unexpectedExits.length >= 3) {
          console.error(
            `[alpha-memory-bridge] 10 分钟内已意外退出 ${this.unexpectedExits.length} 次，` +
  ...
  private async handshake(): Promise<void> {
    try {
      ...
      if (!this.disposed) {
        console.error(`[alpha-memory-bridge] 握手失败: ${(err as Error).message}`)
  ...
  private scheduleRetry(): void {
    ...
    if (this.gaveUp) {
      ...
      this.flushBootQueue(false)
      console.error(
        `[alpha-memory-bridge] 连续启动失败 ${this.retries} 次，已停止自动重启（不再后台空转）。` +
