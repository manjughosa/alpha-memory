...
export function migrateLegacyData() {
    ...
    const out = { ran: false, reason: '', from, to, copied: 0, failed: [] };
    ...
    for (const name of names) {
        ...
        if (existsSync(dst) && !isEmptyDir(dst))
            continue;
        try {
    ...
    if (!out.ran)
        out.reason = out.failed.length ? '目标不可写' : '新位置已有数据，未覆盖';
    return out;
}
export const ENV_CHILD_CWD = 'MDCG_CHILD_CWD';
let cachedRunRoot = null;
/**
 ...
 */
export function mdcgRoot(configured) {
 ...
 */
export function pythonPathValue() {
    const prev = process.env.PYTHONPATH ?? '';
    const parts = [repoRoot()];
    if (prev && !prev.split(delimiter).includes(repoRoot()))
        parts.push(prev);
    return parts.join(delimiter);
}
/** 当前解析快照（供启动日志留痕——把「记忆真源在哪」写进可审计痕迹）。 */
export function describeDataPaths(configured) {
    const dr = dataRoot();
    const mr = mdcgRoot(configured);
    const paths = readUserPaths();
    const pfSource = pathsFileSource();
    let source = 'default(用户级状态根 data/)';
    if (process.env[ENV_DATA_ROOT])
        source = `env:${ENV_DATA_ROOT}`;
    else if (process.env[ENV_MDCG_ROOT])
        source = `env:${ENV_MDCG_ROOT}`;
