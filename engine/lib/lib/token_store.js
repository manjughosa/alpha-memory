...
/** 写入密钥环并尽力收紧权限。 */
function writeKeyring(path, token) {
...
function issueToken(python, role, actor, clearance) {
    let r;
    try {
        r = spawnSync(python, ['-m', 'md_cg.tokens', 'issue',
    ...
    if (r.error)
        return { error: `无法执行 ${python}：${r.error.message}` };
    if (r.status !== 0) {
...
function autoIssueDisabledByEnv() {
...
export function resolveToken(opts) {
    ...
    if (configured)
        return { token: configured, source: 'config', keyringPath };
    const fromEnv = (process.env.MDCG_TOKEN ?? '').trim();
    if (fromEnv)
        return { token: fromEnv, source: 'process-env', keyringPath };
    const fromRing = readKeyring(keyringPath);
    if (fromRing)
        return { token: fromRing, source: 'keyring', keyringPath };
