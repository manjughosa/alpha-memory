// @ts-nocheck
/**
 * alpha-memory 扩展：角色扮演网页（同源挂载到 webServer /roleplay 前缀）。
 *
 * 职责边界（三层拆分后）：
 ...
 *    返回明确原因（不编造回复）。启用见 config.capability（宿主可选能力配置）。
 */
...
<body>
<header>
  <h1>Alpha · 角色扮演</h1><span class="badge">白箱 · 扮演论 v3.3</span>
  <div id="roles">
...
<main id="chat"><div class="hint">选择角色，开始对话。角色回应由Alpha角色扮演引擎生成（白箱优先，LLM 续答）。</div></main>
...
<div id="agegate">
  <div class="box">
    ...
    <div class="warn">本页面提供角色扮演内容，部分角色/对话可能包含<b>成人向（NSFW）</b>内容。<br><br>
      依据国家法律法规与Alpha协议（对人类的保护）：<br>
      ① 仅限<b>已满 18 周岁</b>的用户使用成人向内容；<br>
      ② 仅限<b>个人对话场景</b>（本页面为本地一对一，不公开）；<br>
      ③ Alpha<b>拒绝一切涉及未成年人的性内容</b>——此类请求会被直接拒绝并记录。<br><br>
...
let CLIENT_ID = '';
try {
  CLIENT_ID = localStorage.getItem('alpha_client_id');
  if (!CLIENT_ID) {
    CLIENT_ID = 'c' + Math.random().toString(36).slice(2) + Date.now().toString(36);
    localStorage.setItem('alpha_client_id', CLIENT_ID);
  }
...
const AGE_KEY = 'alpha_roleplay_age_confirmed';
...
function nsfwCheck(text) {
  ...
  if (minor && nsfw) {
    return { refuse: true, msg: '⛔ 已拒绝：涉及未成年人的性内容违反国家法律与Alpha协议（未成年人保护）。Alpha不提供任何涉及未成年人的性扮演内容。' };
  }
 ...
 */
export async function installRoleplayWeb(ctx, capability, config, disposers, mdcg = null) {
    // 身体能力后端未接入时的统一提示（fail-closed：不编造回复）。
    const CAP_ABSENT_MSG = '角色生成能力未接入：可选能力后端未配置。'
        + '当前仅保留转录/历史/翻译/落图。'
    ...
    try {
        ...
        const toGraph = async (label, fn) => {
            ...
            try {
                ...
                ctx.logger.warn(`alpha-memory: 角色扮演落图失败（${label}）：${String(err)}`);
            }
        };
        // P1 修复（GPT 审查）：
        ...
        const writeRoleMeta = (flat) => {
            try {
                ...
                ctx.logger.warn(`alpha-memory: 角色 meta 写入失败: ${String((e && e.message) || e)}`);
            }
        };
        // 翻译模式（双向/仅输入）由网页层持久化
        ...
        // 可逆替换（长词优先，与Alpha translate_input/translate_output 同逻辑）
        ...
        const handler = async (req, res) => {
            ...
            try {
                ...
                const roleItemsKey = (kind) => `${kind}_items`;
                const getRoleItems = (roleId, kind) => {
                    const meta = readRoleMeta();
                    const arr = (meta[roleId] && meta[roleId][roleItemsKey(kind)]) || [];
                ...
                if (req.method === 'POST' && url.pathname.startsWith('/roleplay/api/roles/') && url.pathname.endsWith('/import')) {
                    ...
                    meta[role][roleItemsKey(kind)] = items;
                    writeRoleMeta(meta);
                    ...
                    void toGraph(`import-${kind}`, (g) => Promise.all(items.map((it, i) => g.write(typeof it === 'string' ? it : (it.text || it.content || JSON.stringify(it)), { node_id: `roleplay_${role}_${kind}_${i}`, layer: layerForKind,
                ...
                if (req.method === 'POST' && url.pathname === '/roleplay/api/chat') {
                    ...
                    if (hasMinor && hasNsfw) {
                        res.writeHead(200, { 'Content-Type': 'application/json; charset=utf-8' });
                        res.end(JSON.stringify({ reply: '⛔ 已拒绝：涉及未成年人的性内容违反国家法律与Alpha协议（未成年人保护）。Alpha不提供任何涉及未成年人的性扮演内容。', route: 'refused', refused: 'minor_nsfw' }));
                        return;
                    }
        ...
        if (config.roleplayEntryButton) {
            disposers.push(webServer.tapIndex((html) => {
                if (html.includes('href="/roleplay"'))
                    return html;
                const btn = '<a id="宿主m-rp" href="/roleplay" style="position:fixed;top:12px;right:14px;z-index:999999;background:#7c8cff;color:#0b0d14;border-radius:999px;padding:9px 15px;font:600 13px/1 system-ui,sans-serif;text-decoration:none;box-shadow:0 4px 14px rgba(0,0,0,.45);touch-action:none;user-select:none;cursor:grab">🎭 角色扮演</a>' +
                    '<script>(function(){var b=document.getElementById("宿主m-rp");if(!b)return;' +
                    'try{var p=JSON.parse(localStorage.getItem("宿主m_rlp")||"null");' +
                    ...
                    'localStorage.setItem("宿主m_rlp",JSON.stringify({l:r.left,t:r.top}));}catch(err){}}' +
                    'drag=false;sx=0;sy=0;});})();</script>';
                return html.replace('</body>', `${btn}\n</body>`);
            }));
        }
        ctx.logger.info(`alpha-memory: 角色扮演网页已挂载 /roleplay（data_dir=${roleDataDir}，入口按钮=${config.roleplayEntryButton ? '显示（可拖动）' : '隐藏'}）`);
    }
    catch (err) {
        ctx.logger.warn(`alpha-memory: 角色扮演网页挂载失败（不影响其他功能）: ${String(err && err.message || err)}`);
    }
}
