...
export function initPlaceholders(now = new Date().toISOString()) {
    return {
        initialized: true,
        initializedAt: now,
        slots: {
 ...
 */
export function evaluatePlaceholder(userTurns, placeholder, currentRound, allowedModes, now = new Date().toISOString()) {
...
function detectTopicShift(recent) {
    if (recent.length < 2)
        return false;
...
export function movePlaceholder(state, slot, newAnchor, now = new Date().toISOString()) {
 ...
 * 上限内原样返回，不标注。
 */
export function smartWindow(text, limit = 8000) {
